...
void MaterialManager::displayInfo() {
    cout << endl << "  자재ID  |     자재명     |   공급업체   |   단가   | 재고량 | 주문량 " << endl;
    for (const auto& pair : materialList) {
        Material* m = pair.second;
        cout << setw(7) << m->getId() << " | ";  // 자재 ID 출력
        cout << setw(12) << m->getName() << " | ";  // 자재명 출력
        cout << setw(12) << m->getSupplier() << " | ";  // 공급업체명 출력
        cout << setw(9) << fixed << m->getUnitPrice() << " | ";  // 단가 출력
