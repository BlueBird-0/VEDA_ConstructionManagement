...
void MaterialManager::create() {
    ...
    setCmdColor(0);
    printf("%10s | %16s | %20s | %18s | %20s\n","자재명", "공급업체", "단가(₩)", "재고량(EA)", "주문량(EA)");
...
void MaterialManager::displayInfo() {
	...
	if (!materialList.empty()) {
		printf("%6s | %10s | %16s | %20s | %18s | %20s\n", "자재ID", "자재명", "공급업체", "단가(₩)", "재고량(EA)", "주문량(EA)");

		for (const auto& pair : materialList) {
			Material* m = pair.second;
			cout << setw(6) << m->getId() << " | ";  // 자재 ID 출력
			cout << setw(10) << m->getName() << " | ";  // 자재명 출력
			cout << setw(16) << m->getSupplier() << " | ";  // 공급업체명 출력
			cout << setw(20) << fixed << m->getUnitPrice() << " | ";  // 단가 출력
			cout << setw(18) << m->getStock() << " | ";  // 재고량 출력
			cout << setw(20) << m->getOrderQuantity() << endl;  // 주문량 출력
		}
	}
	else {
		setCmdColor(2);
